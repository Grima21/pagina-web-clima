...
const API_KEY = "7ea3cc2c2bc37e5d0132393f71c915ec"; // Usuario debe reemplazar con su clave de OpenWeatherMap
const BASE_URL = "https://api.openweathermap.org/data/2.5";
const GEO_URL = "http://api.openweathermap.org/geo/1.0";

// Función para verificar si la API key es válida
const isValidApiKey = (key: string): boolean => {
  return key !== "7ea3cc2c2bc37e5d0132393f71c915ec" && key.length > 10;
};

// Función para generar datos mock basados en coordenadas
const generateMockWeatherForLocation = (
  lat: number,
  lon: number,
  cityName?: string
): WeatherData => {
...
export const weatherApi = {
  getCurrentWeather: async (lat: number, lon: number): Promise<WeatherData> => {
    ...
    if (!isValidApiKey(API_KEY)) {
      console.warn(
        "Usando datos de demostración. Para datos reales, obtén una API key de OpenWeatherMap."
      );
